...
async def node_route(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="auto" if decision == "auto_approve" else "escalate" if decision == "escalate" else "pending",
...
async def node_human_approval(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="pending",
        reason="Waiting for reviewer approval decision",
...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        confidence=a.confidence,
        risk_level=risk_level_for(a.confidence),
        decision="approve" if action == "committed" else "reject" if action == "rejected" else "pending",
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason=f"Auto path action: {action}",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="escalate",
        reason=f"Escalating with {len(questions)} question(s)",
        reviewer_id=os.environ.get("GITHUB_USER"),
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_synthesize(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="pending",
        reason="Refined analysis after escalation answers",
        reviewer_id=os.environ.get("GITHUB_USER"),
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
